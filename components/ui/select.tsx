...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-fit w-full items-center justify-between rounded-md border-input bg-primary px-[6px] py-1 text-[9px] placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:px-3 md:py-[8px] md:text-base [&>span]:line-clamp-1",
      className,
    )}
    {...props}
  >
    <div className="flex items-center">
      <SelectPrimitive.Icon asChild>
        <SortDesc className="mr-1 h-3 w-3 opacity-75 md:h-4 md:w-4" />
